      ...
      <div className="flex-1 flex flex-col items-center justify-center py-12 lg:py-6 px-4 sm:px-12 lg:px-16 xl:px-24 overflow-y-auto lg:overflow-hidden no-scrollbar">
        ...
        >
          ...
          <div>
            ...
            <p className="text-white/40 text-sm mt-1.5">Input your basic details to begin the journey.</p>
          </div>
          ...
          <p className="text-sm text-white/40">
            Member of the team?{" "}
            <Link href="/login" className="text-white hover:underline transition-colors ml-1">
              Log in
            </Link>
          </p>
        </motion.div>
...
function SocialButton({ icon: Icon, label, onClick }: { icon: any; label: string; onClick?: () => void }) {
  return (
    <button 
      type="button" 
      onClick={onClick} 
      className="flex items-center justify-center gap-2 bg-black border border-white/10 rounded-xl h-12 hover:bg-white/5 transition-colors cursor-pointer text-sm font-semibold text-white"
    >
