  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 lg:px-12 py-6 flex items-center justify-between border-b border-white/5 bg-black/80 backdrop-blur-md">
        ...
        <span className="text-xs font-mono text-white/30 tracking-widest">FEATURED BUILDERS</span>
      </nav>
      ...
      <section className="relative pt-40 pb-24 px-6 lg:px-12 max-w-[1400px] mx-auto">
        <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <h1 className="text-7xl md:text-[10rem] lg:text-[160px] font-display tracking-tight leading-[0.85] mb-8">
            ELITE
            <br />
            <span className="text-white/20">BUILDERS.</span>
          </h1>
      ...
      <section className="px-6 lg:px-12 pb-40 max-w-[1400px] mx-auto">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {featuredBuilders.map((builder, i) => (
            ...
            >
              {/* Tag */}
              <span className="text-[10px] font-mono text-white/30 tracking-widest mb-6 block">{builder.tag}</span>

              {/* Handle */}
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h2 className="text-2xl font-display">{builder.handle}</h2>
                  <p className="text-sm text-white/40 mt-1">{builder.role}</p>
                </div>
                <div className={`w-8 h-8 flex items-center justify-center border transition-all duration-300 ${
                  hoveredIndex === i ? "border-white bg-white text-black" : "border-white/20"
                }`}>
              ...
              {/* Bio */}
              <p className="text-sm text-white/50 leading-relaxed mb-6">{builder.bio}</p>

              {/* Stack */}
              <div className="flex flex-wrap gap-2 mb-6">
                {builder.stack.map((s) => (
                  <span key={s} className="text-[11px] font-mono px-2 py-1 border border-white/10 text-white/40">
                    {s}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-6 pt-6 border-t border-white/10">
                <div>
                  ...
                  <span className="text-[11px] text-white/30 font-mono">projects</span>
                </div>
                <div>
                  <span className="text-2xl font-display block">{builder.collab}</span>
                  <span className="text-[11px] text-white/30 font-mono">collab rate</span>
                </div>
              </div>
        ...
        <div className={`mt-20 text-center transition-all duration-1000 delay-700 ${isVisible ? "opacity-100" : "opacity-0"}`}>
          <p className="text-white/30 text-sm font-mono mb-6">2,400+ builders on the platform</p>
          <Link
            href="/"
