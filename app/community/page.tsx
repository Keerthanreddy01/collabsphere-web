  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 lg:px-12 py-6 flex items-center justify-between border-b border-white/5 bg-black/80 backdrop-blur-md">
        ...
        <span className="text-xs font-mono text-white/30 tracking-widest">COMMUNITY</span>
      </nav>
      ...
      <section className="relative pt-40 pb-24 px-6 lg:px-12 max-w-[1400px] mx-auto">
        <div className={`transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          ...
          <h1 className="text-7xl md:text-[10rem] lg:text-[160px] font-display tracking-tight leading-[0.85] mb-8">
            WHERE
            <br />
            <span className="text-white/20">BUILDERS LIVE.</span>
          </h1>
      ...
      <section className={`px-6 lg:px-12 pb-24 max-w-[1400px] mx-auto transition-all duration-1000 delay-200 ${isVisible ? "opacity-100" : "opacity-0"}`}>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {stats.map((stat, i) => (
            <div key={stat.label} className="p-8 border border-white/10 bg-white/[0.02]"
              style={{ transitionDelay: `${i * 100}ms` }}>
              <span className="text-5xl font-display block mb-2">{stat.value}</span>
              <span className="text-sm text-white/40 font-mono">{stat.label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 lg:px-12 pb-24 max-w-[1400px] mx-auto">
        <span className="text-xs font-mono text-white/30 tracking-widest mb-12 block">HOW WE BUILD</span>
      ...
      <section className="px-6 lg:px-12 pb-40 max-w-[1400px] mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <span className="text-xs font-mono text-white/30 tracking-widest">LIVE FEED</span>
        ...
        <div className="space-y-2">
          {recentActivity.map((item, i) => (
            <div
              key={i}
              className={`flex items-center justify-between p-5 border border-white/10 hover:border-white/30 transition-all duration-300 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-mono text-white/60">{item.user}</span>
                <span className="text-sm text-white/40">{item.action}</span>
              </div>
              <span className="text-xs text-white/20 font-mono shrink-0">{item.time}</span>
            </div>
          ))}
        </div>
